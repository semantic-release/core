  ...
  const plugins = await getPlugins(
    {
      cwd,
      logger: t.context.logger,
      options: { plugins: [plugin1, [plugin2, {}]], analyzeCommits: () => "patch", verifyRelease: () => {} },
    },
    {}
  );
...
test('Export plugins based on "plugins" config (single definition)', async (t) => {
  const plugin1 = { verifyConditions: stub(), publish: stub() };
  const plugins = await getPlugins(
    { cwd, logger: t.context.logger, options: { plugins: plugin1, analyzeCommits: () => "patch" } },
    {}
  );
  ...
  const plugins = await getPlugins(
    {
      ...
      logger: t.context.logger,
      options: {
        plugins: ["./test/fixtures/multi-plugin.cjs", "./test/fixtures/multi-plugin.cjs"],
        analyzeCommits,
        generateNotes,
        publish,
        success,
        ...
        getPlugins(
          {
            ...
            logger: t.context.logger,
            options: {
              plugins: ["./test/fixtures/multi-plugin.cjs"],
              verifyConditions: 1,
              analyzeCommits: [],
              verifyRelease: [{}],
