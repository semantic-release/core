...
export default async (context, runtimeOptions, { buildPlugins = false, baseConfig } = {}) => {
  const { cwd, env } = context;
  const { config, filepath } = (await cosmiconfig(CONFIG_NAME).search(cwd)) || {};

  debug("load config from: %s", filepath);

  // Merge base config, config file options and caller-provided options
  const resolvedBaseConfig = await resolveBaseConfig(cwd, baseConfig);
  let options = { ...resolvedBaseConfig, ...config, ...runtimeOptions };
