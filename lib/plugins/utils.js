...
import buildPlugins from "./index.js";
import { PLUGIN_PIPELINE_STEPS } from "../definitions/constants.js";
...
export function isPluginsPipeline(plugins) {
  return Boolean(plugins) && PLUGIN_PIPELINE_STEPS.every((step) => typeof plugins[step] === "function");
}

export async function normalizePluginsInput(context, plugins) {
  if (isPluginsPipeline(plugins)) {
    return plugins;
  }

  const options = plugins === undefined ? context.options : { ...context.options, plugins };
  return buildPlugins({ ...context, options }, {});
}
