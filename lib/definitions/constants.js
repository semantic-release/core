export const RELEASE_TYPE = ["patch", "minor", "major"];

export const FIRST_RELEASE = "1.0.0";

export const FIRST_PRERELEASE = "1";

export const COMMIT_NAME = "semantic-release-bot";

export const COMMIT_EMAIL = "32174276+semantic-release-bot@users.noreply.github.com";

export const RELEASE_NOTES_SEPARATOR = "\n\n";

export const SECRET_REPLACEMENT = "[secure]";

export const SECRET_MIN_SIZE = 5;

export const GIT_NOTE_REF = "semantic-release";

export const PLUGIN_PIPELINE_STEPS = [
  "verifyConditions",
  "analyzeCommits",
  "verifyRelease",
  "generateNotes",
  "prepare",
  "publish",
  "addChannel",
  "success",
  "fail",
];

export const DEFAULT_ANALYZE_COMMITS_PLUGIN = "@semantic-release/commit-analyzer";
