...
export function EMISSINGREQUIREDPLUGIN({ type }) {
  const fallbackNotice =
    type === "analyzeCommits"
      ? `For \`analyzeCommits\`, core attempts to use \`@semantic-release/commit-analyzer\` as a fallback when no plugin is configured for that step. This error means no valid \`${type}\` plugin could be resolved.`
      : "";

  return {
    message: `The \`${type}\` plugin is required in core configuration.`,
    details: `The [${type} plugin configuration](${linkify(
      `foundation/plugins#${toLower(type)}-plugin`
    )}) must be explicitly provided when using **@semantic-release/core**.

${fallbackNotice}

Add a plugin for the \`${type}\` step in your release configuration, for example:
...
Or configure the step directly with the \`${type}\` option.`,
  };
}
